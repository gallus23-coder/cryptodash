...
- **Trade drill-down (closed trades)**: clicking a closed trade row toggles an expandable panel. On first expand, calls `GET /api/trade-signal-path/:bot/:tradeId`. Renders a Chart.js line chart of `price_gbp` — one point per signal_history row, green for buy/hold signals, red for sell. Tooltip shows signal, RSI, volume ratio. Header shows entry price, exit price, P&L (£ and %), exit reason. Shows "No signal history available" if `signalPath` is empty (trades before Jul 8 2026). Fetch gated by `data-loaded` — fires once per panel.
- **Trade drill-down (open positions)**: same behavior for rows in the Open Positions section. Header shows entry price, live current_rate, unrealized P&L (£ and %), and an amber "⬤ Open" badge. Live values (`current_rate`, `profit_abs`, `profit_pct`) stamped as `data-*` attrs on the row at render time from the Freqtrade API response. Re-expanding clears `data-loaded` so signal path and P&L re-fetch each time. `renderTradeDetail(panel, data, bot, row)` branches on `trade.is_open` to choose the correct header. Chart instances tracked in `_tradeCharts` Map and destroyed on re-render.
...
- **Freqtrade `populate_entry_trend` pattern — CRITICAL**: Always zero `dataframe['enter_long'] = 0` and `dataframe['enter_tag'] = ''` as the **absolute first two lines** of `populate_entry_trend`, before any other logic. Entry signal must ONLY be set on the last candle using `if entry_conditions.iloc[-1]:` → `dataframe.loc[dataframe.index[-1], 'enter_long'] = 1`. Never use `.loc[entry_conditions, col]` to set entry signals — this marks ALL historical rows where conditions were ever met, causing Freqtrade to act on stale signals even when the current candle fails all checks. Bug confirmed Jul 2026: caused at least 2 bad entries (SOL Jul 3, ETH Jul 4) despite logs showing "no entry" 2 seconds before trade placed.
